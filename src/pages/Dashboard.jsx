      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xl font-bold text-gray-900 tracking-tight">KindLink</span>
          </div>
        ...
        {donations.filter(d => d.donatorId === currentUser.id).length === 0 ? (
          ...
          <div className="space-y-4">
            {donations.filter(d => d.donatorId === currentUser.id).map(donation => {
              ...
              return (
                <div key={donation.id} className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm flex items-center gap-4">
                  {donation.image ? (
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500">To: {ngo ? ngo.name : 'Unknown NGO'}</p>
...
function ReceiverDashboard({ donations, currentUser }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-1">Incoming Donations</h2>
        <p className="text-gray-500 text-sm">Manage donations directed to your organization.</p>
      </div>

      {myDonations.length === 0 ? (
        <div className="glass rounded-2xl p-12 text-center flex flex-col items-center justify-center text-gray-500">
            ...
            <div key={donation.id} className="glass rounded-2xl overflow-hidden flex flex-col transition-all hover:shadow-2xl hover:-translate-y-1">
              {donation.image ? (
                <div className="w-full h-48 bg-gray-100 relative">
                  ...
                  <div className="absolute top-3 left-3 px-3 py-1 bg-white/90 backdrop-blur text-sm font-semibold text-brand-700 rounded-full shadow-sm">
                    {donation.category === 'Other' ? donation.customCategory : donation.category}
                  </div>
                </div>
              ) : (
              ...
              <div className="p-5 flex-1 flex flex-col">
                <div className="flex-1 space-y-3">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900">{new Date(donation.dateSlot).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
