...
function DonatorDashboard({ ngos, addDonation, currentUser, donations, removeDonation }) {
  ...
  const handleSubmit = (e) => {
    ...
    const donationData = {
      ngoId: selectedNgo,
      donatorId: currentUser?.id || currentUser?.credential || 'unknown_user',
      category,
    ...
    setTimeout(() => {
      ...
      setSuccess(true);
      setShowHistory(true); // Automatically expand the history to show the newly added donation
          ...
          {showHistory && (
            ...
            >
              {donations.filter(d => d.donatorId === (currentUser?.id || currentUser?.credential || 'unknown_user')).length === 0 ? (
                ...
                >
                  {donations.filter(d => d.donatorId === (currentUser?.id || currentUser?.credential || 'unknown_user')).map(donation => {
                    const ngo = ngos.find(n => n.id === donation.ngoId);
                    return (
                      <motion.div 
                        variants={listItem}
                        key={donation.id} 
                        className="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col gap-4"
                      >
                        <div className="flex items-center gap-4">
                          {donation.image ? (
                          ...
                          <div className="text-right flex flex-col items-end">
                            {donation.status !== 'verified' && (
                              <button
                                ...
                                className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-md transition-colors inline-flex items-center justify-center"
                        ...
                        {/* 3-Step Tracker */}
                        <div className="mt-2 pt-5 border-t border-gray-100 dark:border-gray-700 relative">
                           <div className="flex justify-between items-center relative z-10">
                             {['pending', 'in_transit', 'verified'].map((step, idx) => {
                               const statuses = ['pending', 'in_transit', 'verified'];
                               const currentIdx = statuses.indexOf(donation.status || 'pending');
                               const isCompleted = idx <= currentIdx;
                               const isCurrent = idx === currentIdx;
                               const labels = ['Order Collected', 'In Transit', 'Received in NGOs'];
                               return (
                                 <div key={step} className="flex flex-col items-center gap-2 relative z-10 bg-white dark:bg-gray-800 px-2">
                                   <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold border-2 transition-colors ${
                                     isCompleted ? 'bg-brand-500 border-brand-500 text-white shadow-md shadow-brand-500/20' : 'bg-gray-50 border-gray-200 text-gray-400 dark:bg-gray-900 dark:border-gray-700'
                                   }`}>
                           ...
                           <div className="absolute top-[34px] left-8 right-8 h-1 bg-gray-100 dark:bg-gray-700 rounded-full z-0 overflow-hidden">
                             <motion.div 
                               className="h-full bg-brand-500 transition-all duration-700 ease-out" 
                               initial={{ width: '0%' }}
                               animate={{ width: donation.status === 'verified' ? '100%' : donation.status === 'in_transit' ? '50%' : '0%' }}
                             />
                           </div>
                        </div>
                      </motion.div>
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    <div className="flex-1 space-y-4">
                      ...
                      <div className="pt-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
                        <span className={`inline-flex items-center gap-1.5 py-1.5 px-3 rounded-lg text-xs font-bold shadow-sm ${
                          donation.status === 'verified' 
                          ...
                          {donation.status === 'verified' ? (
                            ...
                            <motion.span 
                              ...
                              className={`w-1.5 h-1.5 rounded-full ${donation.status === 'in_transit' ? 'bg-blue-500' : 'bg-yellow-500'}`} 
                            />
                          )}
                          {donation.status === 'verified' ? 'Received' : donation.status === 'in_transit' ? 'In Transit' : 'Pending Pickup'}
                        </span>
                        {donation.location && (
                          <a 
                    ...
                    <div className="mt-5 flex gap-3">
                      <motion.button 
                        ...
                        onClick={() => {
                          const nextStatus = donation.status === 'pending' || !donation.status ? 'in_transit' : 'verified';
                          updateDonationStatus(donation.id, nextStatus);
                        }}
                      ...
                      >
                        {donation.status === 'verified' ? 'Verified Successfully' : donation.status === 'in_transit' ? 'Mark as Received' : 'Mark as In Transit'}
                      </motion.button>
                      <motion.button
                        whileTap={{ scale: 0.97 }}
                        onClick={() => removeDonation(donation.id)}
